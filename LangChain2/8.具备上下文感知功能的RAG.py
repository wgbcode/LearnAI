...
知识点：
1、步骤一：构建向量知识库
2、步骤二：创建一个检索器，增强用户提问，生成一个新的 prompt。传入参数：大模型；向量知识库检索器；上下文（系统提示+用户输入+历史记录）
3、步骤三：创建 RAG 检索链
（1）先创建文档处理链（系统提示+用户输入+历史记录）
（2）再创建 RAG 检索链：检索器=>文档处理链（核心）
（3）为整个检索链添加上下文功能（RunnableWithMessageHistory）
"""

# TODO:版本问题：langchain.chains 包不存在
import bs4
...
from test_packages.custom_embedding import CustomQwen3Embeddings
